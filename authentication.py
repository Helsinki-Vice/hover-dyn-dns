...
def init_session(proxies, logging: bool) -> requests.cookies.RequestsCookieJar:
    """
...
def submit_username_password(username: str, password: str, cookies: requests.cookies.RequestsCookieJar, proxies, logging: bool) -> requests.Response:
    """
...
def submit_2fa_code(totp_code, cookies, proxies, logging: bool):
    """
...
def login(username: str, password: str, totp_secret: str, proxies, logging: bool) -> requests.cookies.RequestsCookieJar | None:

    log_output("Logging in to hover...", logging)
    # Check if we are logged in already, reuse session if so
    cookies = load_cookies(COOKIES_FILE)
    if check_cookies_validity(cookies, proxies):
        log_output("Skipping login process because we are already logged in.", logging)
        return cookies
    
    # Otherwise, log in from scratch
    cookies = init_session(proxies, logging)
    try:
        login_response = submit_username_password(username, password, cookies, proxies, logging)
        login_response_cookies = login_response.cookies
        login_response = login_response.json()
    except json.JSONDecodeError:
        log_output(f"Could not login: the endpoint {AUTH_ENDPOINT} did not return JSON data as expected.", logging)
        exit()
    
    if not isinstance(login_response, dict):
        log_output(f"Could not login: the endpoint {AUTH_ENDPOINT} returned {login_response} instead of valid JSON.", logging)
        exit()
    
    login_error = login_response.get("error")
    if login_error:
        log_output(f"Could not login: the endpoint {AUTH_ENDPOINT} returned the following error: {login_response}", logging)
        exit()
    ...
    if login_response.get("status") == "completed":
        ...
        if auth_type == "app":
            ...
            try:
                totp_response = submit_2fa_code(totp, cookies, proxies, logging)
                cookies = totp_response.cookies
                totp_response = totp_response.json()
            except json.JSONDecodeError:
                log_output(f"Could not submit 2fa login code: the endpoint {AUTH_2FA_ENDPOINT} did not return JSON data as expected.", logging)
                exit()

            if not isinstance(totp_response, dict):
                log_output(f"Could not login: the endpoint {AUTH_2FA_ENDPOINT} returned {totp_response} instead of valid JSON.", logging)
                exit()
            if totp_response.get("succeeded") != True:
                log_output(f"Could not login: totp code {totp} was not accepted. The server's response was was: {totp_response}", logging)
                exit()
        elif auth_type == None:
            pass
        else:
            log_output(f"Hover is asking for a 2FA type of '{auth_type}', which we cannot handle. In your account settings, disable 2FA and re-enable as app authentication. See the readme for more details.", logging)
            exit()
    ...
    if check_cookies_validity(cookies, proxies):
        ...
        log_output(f"The login process seemed to go well, but the server is not accepting our auth cookie of {cookies.get('hoverauth')}.", logging)
        exit()